...
const longProcess = async (body) => {
  ...
  var obj;
  obj = fs.readFileSync(body.obj, "utf8");
  obj = JSON.parse(obj);
  var obj1;
  obj1 = fs.readFileSync(body.obj1, "utf8");
  obj1 = JSON.parse(obj1);
  var obj2;
  obj2 = fs.readFileSync(body.obj2, "utf8");
  obj2 = JSON.parse(obj2);

  admin_amount_total = 0
  var admin_arr = [];
  console.log("begginnig.....")
  ...
  for (let k = 0; k < obj1[0].data.length; k++) {
    if (k == 0) {
      ...
      if (obj1[0].data[k].length > 0) {
        ...
        cashfree_arr[k - 1][Cashfree_head.length + 1] = "N/a"
        cashfree_arr[k - 1][Cashfree_head.length + 2] = "N/a"
        cashfree_arr[k - 1][Cashfree_head.length + 3] = "N/a"
        cashfree_amount_total = cashfree_amount_total + parseInt(obj1[0].data[k][cashfree_amount])
      }
    }
  }
  cashfree_arr.sort(sortNumber1);

  function sortNumber1(a, b) {
    return a[cashfree_id] - b[cashfree_id];
  }
  ...
  for (let k = 0; k < obj2[0].data.length; k++) {
    ...
    if (k == 0) {
      ...
      if (obj2[0].data[k].length > 0) {
        if (obj2[0].data[k][bank_amount] == Number(obj2[0].data[k][bank_amount])) {
        ...
        let s1;
        if (matches===null || matches == '' || typeof matches == 'undefined') {
          s1= 0;
        }else{
          s1 =matches[0] ;
        }

        obj2[0].data[k][bank_id] = s1;
        bank_arr.push(obj2[0].data[k]);
        bank_arr[k - 1][Bank_head.length + 1] = "N/a"
        bank_arr[k - 1][Bank_head.length + 2] = "N/a"
        bank_arr[k - 1][Bank_head.length + 3] = "N/a"
  ...
  for (let i = 0; i < len1; i++) {
    ...
    if (res != -1) {
      ...
      if (admin_arr[i][admin_amount] == res[0][cashfree_amount]) {
        ...
        cashfree_arr[res[1]][Cashfree_head.length - 3 + 1] = admin_arr[i][admin_cashfree_id];
        cashfree_arr[res[1]][Cashfree_head.length - 3 + 2] = admin_arr[i][admin_amount];
        cashfree_arr[res[1]][Cashfree_head.length - 3 + 3] = cashfree_amount_diff1;
        ...
        if (check_browser_count_for_cashfree < 500) {
          admin_html = admin_html + `<tr ><td class="w15">` + res[0][cashfree_id] + `</td><td class="w15">` + res[0][cashfree_amount] + `</td><td class="w15">` + cashfree_amount_diff + `</td></tr>`;
          cashfree_html = cashfree_html + `<tr><td class="w15">` + admin_arr[i][admin_cashfree_id] + `</td><td class="w15">` + admin_arr[i][admin_amount] + `</td><td class="w15">` + cashfree_amount_diff1 + `</td></tr>`;
        }
      }
      if (cashfree_flag == 0) {
        ...
        if (check_browser_count_for_cashfree1 < 500) {
          admin_html = admin_html + `<tr><td class="w15">` + admin_arr[i][admin_cashfree_id] + `</td><td class="w15">` + admin_arr[i][admin_amount] + `</td><td class="w15">` + cashfree_amount_diff + `</td></tr>`;
          cashfree_html = cashfree_html + `<tr><td class="w15">` + admin_arr[i][admin_cashfree_id] + `</td><td class="w15">` + res[0][cashfree_amount] + `</td><td class="w15">` + cashfree_amount_diff1 + `</td></tr>`;
        }
        admin_count = admin_count + 1;
        flag = 1;
      }
    ...
    if (flag == 0 && cashfree_flag == 0) {
      ...
      if (check_browser_count_for_cashfree2 < 500) {
        admin_html = admin_html + `<tr><td class="w15">` + admin_arr[i][admin_cashfree_id] + `</td><td class="w15">` + admin_arr[i][admin_amount] + `</td><td class="w15">N/a</td></tr>`;
      }
    }
  ...
  for (let i = 0; i < len1; i++) {
    ...
    if (res != -1) {
      ...
      if (admin_arr[i][admin_amount] == res[0][bank_amount]) {
        ...
        bank_arr[res[1]][Bank_head.length - 3 + 1] = admin_arr[i][admin_bank_id]
        bank_arr[res[1]][Bank_head.length - 3 + 2] = admin_arr[i][admin_amount]
        bank_arr[res[1]][Bank_head.length - 3 + 3] = bank_amount_diff1
        ...
        if (check_browser_count_for_bank < 500) {
          admin_html = admin_html + `<tr><td class="w15">` + res[0][bank_id] + `</td><td class="w15">` + res[0][bank_amount] + `</td><td class="w15">` + bank_amount_diff + `</td></tr>`;
          bank_html = bank_html + `<tr><td class="w15">` + admin_arr[i][admin_bank_id] + `</td><td class="w15">` + admin_arr[i][admin_amount] + `</td><td class="w15">` + bank_amount_diff1 + `</td></tr>`;
        }
      }
      if (bank_flag == 0) {
        ...
        if (check_browser_count_for_bank1 < 500) {
          admin_html = admin_html + `<tr><td class="w15">` + admin_arr[i][admin_bank_id] + `</td><td class="w15">` + admin_arr[i][admin_amount] + `</td><td class="w15">` + bank_amount_diff + `</td></tr>`;
          bank_html = bank_html + `<tr><td class="w15">` + admin_arr[i][admin_bank_id] + `</td><td class="w15">` + res[0][bank_amount] + `</td><td class="w15">` + bank_amount_diff1 + `</td></tr>`;
        }
        admin_count = admin_count + 1;
        flag = 1;
      }
    ...
    if (flag == 0 && bank_flag == 0) {
      ...
      if (check_browser_count_for_bank2 < 500) {
        admin_html = admin_html + `<tr><td class="w15">` + admin_arr[i][admin_bank_id] + `</td><td class="w15">` + admin_arr[i][admin_amount] + `</td><td class="w15">N/a</td></tr>`;
      }
    }
  ...
  for (let i = 0; i < bank_duplicate_check.length; i++) {
    if (i % 2 == 0) {
      process.send({
        isCompleted: false,
        total_count: len1 + len1 + bank_duplicate_check.length,
        data_1: len1 + i,
        data: len1
      })
    }
    ...
    let res = binarySearch2(cashfree_duplicate_check, len, bank_id, cash_id, type = 3);
  ...
  function binarySearch2(arr, len2, bank_id, cash_id, type) {
    let start = 0;
    let stop = len2 - 1;
    let middle = Math.floor((start + stop) / 2)
    let temp_arr = [];
    if (type == 3) {
      ...
      while (parseInt(arr[middle][admin_cashfree_id]) !== parseInt(cash_id) && parseInt(arr[middle][admin_bank_id]) !== parseInt(bank_id) && start < stop) {

        if (bank_id < arr[middle][admin_bank_id]) {
          stop = middle - 1
        } else {
          start = middle + 1
        }
      ...
      if (parseInt(arr[middle][admin_bank_id]) !== bank_id) {
        return -1;
      } else if (parseInt(arr[middle][admin_bank_id]) == bank_id && parseInt(arr[middle][admin_cashfree_id]) == cash_id) {
        temp_arr = []
  ...
  // =================================================================================
  // ===========================CSV================================
  const dir = 'public/withdraw/' + body.userName;
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir);
  }
// remove upload users files
const directory = 'public/uploads/' + body.userName;
  ...
  try {
    const Admin_csv_file = await convertArrayToCSV(admin_arr, {
      separator: ',',
    });
    let admin_file = fs.writeFileSync("public/withdraw/" + body.userName + "/admin.csv", Admin_csv_file);

    const Cash_free_csv = await convertArrayToCSV(cashfree_arr, {
      separator: ',',
    });
    let cashfree_file = fs.writeFileSync("public/withdraw/" + body.userName + "/cashfree.csv", Cash_free_csv);

    const Bank_csv = await convertArrayToCSV(bank_arr, {
      separator: ',',
    });
    let bank_file = fs.writeFileSync("public/withdraw/" + body.userName + "/bank.csv", Bank_csv);
